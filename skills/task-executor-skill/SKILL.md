...
若用户在会话中直接向你下达指令（未经 Planner 分配子任务），**必须先登记**：

```bash
st direct "指令描述" --desc "详细说明" --session <会话ID>
```

系统会自动创建/复用「直接指令」父任务，并将新子任务指派给你（状态: assigned）。

**完整流程（与正式子任务完全一致）：**

```bash
st start <sub_task_id> --session <会话ID>   # assigned → in_progress
# ... 完成工作 ...
st submit <sub_task_id>                     # in_progress → review（等待 Reviewer 审核）
```

直接指令子任务**不跳过审核**：Reviewer 照常评分，rework/done 流程与正式任务相同。

> 不登记直接执行 = 工作不可追踪，违反系统规范。
